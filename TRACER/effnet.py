...
# SOD
from TRACER.attention import FrequencyEdgeModule
...
def round_filters(filters, global_params):
    """Количество фильтров для EfficientNet
    Args:
        filters (int): исходное количество фильтров
        global_params (namedtuple): кортеж параметров
    Returns:
        new_filters: количество фильтров после вычислений
    """
    multiplier = global_params.width_coefficient
    if not multiplier:
        return filters
    divisor = global_params.depth_divisor
    min_depth = global_params.min_depth # мин числоо фильтров после масштабирования
    filters *= multiplier
    min_depth = min_depth or divisor  # мин число фильтров хотя бы divisor
...
def round_repeats(repeats, global_params):
    """Отвечает за глубину сети
    Args:
        repeats (int): количество повторов
        global_params (namedtuple): параметры модели
    Returns:
        new repeat: новое число повторов
    """
...
def drop_connect(inputs, p, training):
    """аля Dropout
    Inputs: тензор [B, C, H, W], 
                    B - размер батча
                    C - количество каналов
                    H, W - высота и ширина
    """

    # вероятность от 0 до 1 должна быть
    assert 0 <= p <= 1
...
def get_width_and_height_from_size(x):
    """Переводим размеры к единому виду
    Returns:
        size: размер в виде пары чисел [H, W]
    """
...
def calculate_output_image_size(input_image_size, stride):
    ...
    Returns:
        output_image_size: пара чисел [H,W].
    """
...
class Conv2dDynamicSamePadding(nn.Conv2d):
    """Аналог свертки с 'same' в TensorFlow"""
    ...
    def forward(self, x):
        ...
        # Применяем паддинг для нужного размера
...
class Conv2dStaticSamePadding(nn.Conv2d):
    # Как Conv2dDynamicSamePadding, только чуть по-другому паддинг вычисляет
...
def efficientnet_params(model_name):
    """Версии с разными параметрами"""

    params_dict = {
        # Коэфы:   width, depth, res, dropout
...
# Обучение с Adversarial Examples
url_map_advprop = {
